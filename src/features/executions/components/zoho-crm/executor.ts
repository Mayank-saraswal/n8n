...
export const zohoCrmExecutor: NodeExecutor = async ({ nodeId, context, step, publish }) => {
  ...
  return await step.run(`zoho-crm-${nodeId}-execute`, async () => {
    await publish(zohoCrmChannel(nodeId)().status({ status: "loading", nodeId }))
    ...
    try {
      ...
      await publish(zohoCrmChannel(nodeId)().status({ status: "success", nodeId }))
      return { ...context, [variableName]: result }

    } catch (err) {
      await publish(zohoCrmChannel(nodeId)().status({ status: "error", nodeId }))
